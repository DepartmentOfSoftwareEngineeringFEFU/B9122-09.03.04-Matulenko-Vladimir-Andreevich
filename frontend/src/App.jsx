...
function App() {
  ...
  const displayWindStations = weatherData?.wind_stations?.length > 0 ? weatherData.wind_stations : liveWindStations;
  const dataSourceMode = weatherData?.wind_stations?.length > 0 ? 'custom' : 'live';

  useEffect(() => {
    if (weatherData?.wind_stations?.length > 0) {
      setIsLiveLoading(false);
      setLiveError(null);
      return;
    }
    fetchLiveWeather(centerLat, centerLon);
  }, [centerLat, centerLon, fetchLiveWeather, weatherData]);
